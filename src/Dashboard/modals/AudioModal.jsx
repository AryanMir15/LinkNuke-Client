...
const AudioModal = ({ closeModal }) => {
  ...
  const validate = () => {
    ...
    if (files.length > 0 && files[0].size > MAX_AUDIO_SIZE_BYTES)
      errs.file = `Audio must be under ${MAX_AUDIO_SIZE_MB}MB`;
  ...
  const handleSubmit = async (e) => {
    ...
    if (files.length > 0) {
      ...
      try {
        ...
        setErrors({ file: "Failed to upload audio. Try again." });
        setUploading(false);
        return;
      }
      setUploading(false);
          ...
          <div className="relative z-10">
            ...
            <div className="text-center">
              ...
              <p className="mb-6 text-gray-300/80 text-sm sm:text-base">
                Your audio link is ready to share:
              </p>
  ...
  return (
    <div
      className={`w-full max-w-4xl mx-auto rounded-2xl shadow-2xl p-8 flex flex-col gap-8 min-h-[500px] transition-all duration-300 relative overflow-hidden ${
        window.innerWidth < 768
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-2">
          ...
          <InfoModal
            ...
            title="Extra Secure for Audio"
          >
            <div className="text-gray-200">
              When enabled, your audio will be playback-only. Download,
        ...
        <div className="flex justify-between items-center mb-2">
          <h3 className="text-xl font-semibold text-white/80">Upload Audio</h3>
          <button
            onClick={closeModal}
              ...
              <div className="rounded-lg p-4">
                <div className="flex flex-row flex-wrap gap-3 w-full justify-center">
                  {files.map((file, index) => (
                    <div
                      key={index}
                      className="relative w-20 h-20 rounded-lg overflow-hidden border border-gray-600 bg-gray-800 shadow-md flex items-center justify-center"
                    >
                      <span className="truncate text-xs text-gray-300 px-2 text-center w-full">
                        {file.name}
                      </span>
                      <button
